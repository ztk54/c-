...
//    for (int m = 1; m < i; m++)
//    {
//        if (score[m] > max_val) max_val = score[m];
//        if (score[m] < min_val) min_val = score[m];
//    }
//
//    int max_indices[n];
//    int min_indices[n];
...
//    for (int m = 0; m < i;m++)
//    {
//        if (score[m] == max_val) max_indices[max_count++] = m;
//        if (score[m] == min_val) min_indices[min_count++] = m;
//    }
//
//    if (max_count>1||min_count>1)//存在重复值
//    {
//        printf("\n选择处理重复值的方式:\n");
